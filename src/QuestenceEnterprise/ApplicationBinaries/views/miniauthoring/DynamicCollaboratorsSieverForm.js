...
class InstructorGeneratedPills extends React.Component {
  ...
  handleKeypress(event) {
    if (event.key == "Enter") {
      ...
      if (
        matchedInstructor?.email &&
        matchedInstructor?.email === currentcontent
      ) {
        ...
        toast.success(
          `${currentcontent} has been added as collaborator for this course lead Instructor`
        );
        return true;
      } else {
  ...
  makeAddedList() {
    const elements = this.state.myItems.map((listitem, index) => (
      <div>
        <aside
          style={{ position: "relative", float: "left", marginTop: "50px" }}
        >
          <header>
            <a
              href={
                process.env.PUBLIC_URL + "/admin/instructors/" + listitem.id
              }
            >
  ...
  render() {
    return (
      <div className="dynamic_input">
        ...
        <p style={{ background: "#f5f5f5", margin: "10px", padding: "4px" }}>
          Note**: You can search for an instructors who will collaborate on this
          course by email
          <br />
          <span>
            (Continue this process for other collaborators by adding their
            email)
          </span>
          <br />
          <span>(Enter an email and then hit enter )</span>
        </p>
        {this.makeAddedList()}
