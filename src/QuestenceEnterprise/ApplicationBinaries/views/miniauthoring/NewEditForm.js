...
import { updateCourse,getInstructors } from "../../api/enrollment_services/courses.services";
...
const CreateCourse = (props) => {
  ...
  initialValues.enrollment_end = formatDate(
    new Date(initialValues.enrollment_end)
  );
  initialValues.enrollment_start = formatDate(
    new Date(initialValues.enrollment_start)
  );
  ...
  const [previewSrc2, setPreviewSrc2] = useState(
    initialValues.course_cover_image
  );
  ...
  useEffect(() => {
    (async function retrieveFileObejectsFromSource() {
      ...
      let coverImage = await retrieveFileObject(
        initialValues.course_cover_image
      );
  ...
  const [htmlPrerequisites, setHtmlPrerequisites] = useState(
    initialValues.prerequisite_course
  );
