...
const CreateCourse = (props) => {
  ...
  const handleTopics = (e) => {
    ...
    const topicBag =
      localStorage && JSON?.parse(localStorage.getItem("syllables"));
    ...
    if (e.keyCode === 13) {
      ...
      toast.success(topic.title + " has been added to curriculum");
      // setTopics([...syllables]);
  ...
  const fetchContent = async () => {
    Promise.all(
      ...
      .then((res) => {
        ...
        setInstructors([...res[4].data.data.data]);
        setLoading(false);
      })
      .catch((err) => {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    const allCollaborators =
      localStorage && JSON?.parse(localStorage.getItem("instructors_list"));
    ...
    Object.keys(values).forEach((item) => {
      if (!values[item]) {
        if (values[item] !== "course_cover_image") {
          error = true;
          ...
          <Container>
            <Col lg="12">
              <section className="course">
                ...
                <br />
                <div className="">
                  <h3 style={{ fontSize: "22px", marginLeft: "10px" }}>
                    Create Course{" "}
                  </h3>
                </div>
                <form
                ...
                >
                  ...
                  <p>
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        handleSelectedInstructors(e.target.selectedOptions);
                      }}
                    >
                      <option selected={true}>-- Instructors --</option>
                      {instructorList.length > 0 &&
                        instructorList.map((teacher, i) => {
                          return (
                            <option key={i} value={teacher?.id}>
                        ...
                        instructorsCheckSum.map((item, i) => {
                          return (
                            <li key={i} className="added-auto">
                              {getInstructorNameById(item)} has been added as
                              collaborator
                              <button
                                style={{
                                  borderRadius: "20%",
                  ...
                  <p>
                    ...
                    />
                    <ul className="topics">
                      {JSON.parse(localStorage.getItem("syllables")).length >
                        0 &&
                        JSON.parse(localStorage.getItem("syllables")).map(
                          (item, i) => {
                            return (
                              <li
                                key={i}
                              ...
                              >
                                {item.title}
                                {/*<p style={{ borderRadius:"50%",backgroundColor:"red", float:"right"}}
