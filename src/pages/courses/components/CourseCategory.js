...
const CourseCategory = (props) => {
  const { course: { courses }, setFilterAllCourses } = props
 ...
 const history = useHistory()
  ...
  const filterCategoryByFilterBtn = () => {
    ...
    if(document.getElementById("paid-2") && document.getElementById("paid-1") && categoriesClone?.length > 0){
        let price_free = document.getElementById("paid-2")
        let price_paid = document.getElementById("paid-1")
        let isChecked = price_free?.checked ? price_free?.checked : (price_paid?.checked ? price_paid?.checked : false)
        let elementChecked = price_free?.checked ? price_free : (price_paid?.checked ? price_paid : null)
    ...
    if(document.getElementById("paid-11") && document.getElementById("paid-12") && categoriesClone.length > 0){
         let learning_style_self = document.getElementById("paid-11");
         let learning_style_instructor = document.getElementById("paid-12")
        let isChecked = learning_style_self.checked ? learning_style_self.checked : (learning_style_instructor.checked ? learning_style_instructor.checked : false)
        let elementChecked = learning_style_self.checked ? learning_style_self : (learning_style_instructor.checked ? learning_style_instructor : null)
        //we have element checked and the value then we switch between the element and values
        if (isChecked ===true && elementChecked!== null) {
               //checked
  ...
  const filterCategory = (e) => {
    ...
    if (isChecked) {
      ...
      let uniqueDeleted = batchedCategories.filter( ids => parseInt(ids) !== parseInt(e.target.value));
  ...
  return (
    <Fragment>
      ...
      <div className="course-search" >
        ...
        <button style={{color:"#fff",background:"#0253c8",height:"40px"}} className="act-cat"  onClick={filterCategoryByFilterBtn} value="Reset" id="btnUncheckAll">
          <span style={{marginTop:"-30px",padding:"5px"}}>Filter</span>

        </button>


         <button style={{height:"40px",background:"#fff"}} className="act-cat"  onClick={handleReset} value="Reset" id="btnUncheckAll">
          <span style={{marginTop:"-30px",padding:"5px",background:"#fff"}}>Reset</span>
        </button>
</div>
